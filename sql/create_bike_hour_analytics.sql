CREATE OR REPLACE TABLE `{project_id}.{dataset_id}.bike_hour_analytics`
PARTITION BY DATE(event_timestamp)
CLUSTER BY hour_of_day, is_weekend, weather_situation
AS
SELECT
    *,
    CAST(dteday AS DATE) AS event_date,
    TIMESTAMP_ADD(
        TIMESTAMP(CAST(dteday AS DATE)),
        INTERVAL CAST(hr AS INT64) HOUR
    ) AS event_timestamp,

    2011 + CAST(yr AS INT64) AS year,
    CAST(mnth AS INT64) AS month,
    CAST(hr AS INT64) AS hour_of_day,
...
FROM `{project_id}.{dataset_id}.bike_hour_raw`;